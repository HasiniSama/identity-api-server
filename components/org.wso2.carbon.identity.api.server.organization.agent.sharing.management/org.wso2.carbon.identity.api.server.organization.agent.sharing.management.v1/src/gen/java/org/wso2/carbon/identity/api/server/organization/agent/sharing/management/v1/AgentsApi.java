...
public class AgentsApi  {
    ...
    @Produces({ "application/json" })
    @ApiOperation(value = "Perform incremental role assignment operations for already shared agents", notes = "Perform **incremental updates** to the shared attributes (e.g., role assignments) of already shared agents.  This endpoint supports **SCIM-like PATCH semantics**: - `op: \"add\"`    → assign additional roles. - `op: \"remove\"` → remove specific roles.  > **Note:** > Only **role assignment** updates are managed here as of now. Sharing/unsharing organizations > is handled via `/agents/share`, `/agents/share-with-all`, `/agents/unshare`, > and `/agents/unshare-with-all`.  **Scope required:** `internal_agent_shared_access_update`", response = ProcessSuccessResponse.class, authorizations = {
        @Authorization(value = "BasicAuth"),
        @Authorization(value = "OAuth2", scopes = {
